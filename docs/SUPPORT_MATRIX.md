...
| ESP32-S3 gateway-head | ESP-IDF 5.2+ | `idf.py`, 実機ボード | prototype | TinyUSB 有効 build では real backend prototype を優先。初期化失敗時は scaffold として dev backend にフォールバック。compile/HIL は未確認 |
