...
| ESP32-S3 sleepy leaf | ESP-IDF 5.2+ | `idf.py`, 実機ボード | prototype | binary on-air の state/pending-digest/tiny-poll/compact-command/result を使う。event/heartbeat codec は protocol/Host Agent 側で active だが、sleepy runtime の event/heartbeat uplink はまだ限定的。CI smoke compile はあるが deep sleep / HIL は未完了 |
...
| Node runtime scaffold | prototype | development backend smoke | synthetic digest/command を binary on-air で 1 回たどる最小スモーク。event/heartbeat body codec は実装済みだが、sleepy app loop への統合は継続中 |
