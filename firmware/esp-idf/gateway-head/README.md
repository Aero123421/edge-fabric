...
- `main.c` は scaffold として `gateway_head_runtime_use_default_backends()` を呼んでいます
- 実機 backend に差し替える場合は
  `gateway_head_runtime_init_transport() -> usb_link_install_backend(...) -> radio_hal_install_backend(...) -> gateway_head_runtime_start()`
  の順で呼びます
